    ...
    return (
        <>
            ...
            <ReactTooltip id={name} place="right" effect="float" type="light">
                <ul>Strong against: {renderList(name, STRENGTHS).length ? renderList(name, STRENGTHS) : 'none'}</ul>
                <ul>Weak against: {renderList(name, WEAKNESSES).length ? renderList(name, WEAKNESSES) : 'none'}</ul>
                <ul>Immune against: {renderList(name, IMMUNES).length ? renderList(name, IMMUNES) : 'none'}</ul>
            </ReactTooltip>
        </>
    );
};
