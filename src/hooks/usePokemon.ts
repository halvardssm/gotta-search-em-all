...
const usePokemon = () => {
    ...
    async function changePokemon(pokemonIndex: number) {
        try {
            ...
            const speciesData: SpeciesDataProps = await fetch(speciesRequest).then((response) => response.json());
            ...
            const evolutionData = await fetch(evolutionRequest).then((response) => response.json());
            ...
            Promise.all(evolutions).then((responses) => {
                Promise.all(responses.map((response) => response.json())).then((pokemons) => {
